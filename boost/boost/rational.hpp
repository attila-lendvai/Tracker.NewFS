...
#include <boost/config.hpp>     // for BOOST_NO_STDC_NAMESPACE
...
rational<IntType>& rational<IntType>::operator*= (const rational<IntType>& r)
{
    ...
    IntType gcd1 = gcd(num, r.den);
    IntType gcd2 = gcd(r.num, den);
...
{
    // Avoid overflow and preserve normalization
    IntType gcd1 = gcd(num, r.num);
    IntType gcd2 = gcd(r.den, den);
